...
class DRAM():
    def __init__(self, name, config_path = ""):
        ...
        if self.model_type == "HeteroCIM":
            self.size = config.getfloat("HeteroCIM model", "size")
            self.energy_per_bit = config.getfloat("HeteroCIM model", "energy_per_bit")
            self.bit_width = config.getint("HeteroCIM model", "bit_width") if config.getint("HeteroCIM model", "bit_width") != -1 else 512
            self.read_access_latency = config.getfloat("HeteroCIM model", "read_access_latency")
            self.write_access_latency = config.getfloat("HeteroCIM model", "write_access_latency")
            self.DRAM_frequency = config.getfloat("HeteroCIM model", "DRAM_frequency")
            ...
            self.DRAMsim3_trace_gen_path = self.DRAMsim3_dir + "HeteroCIM_scripts/trace_gen_HeteroCIM.py"
            self.DRAMsim3_trace_dir_path = self.DRAMsim3_dir + "HeteroCIM_traces/"
            self.DRAMsim3_res_dir_path = self.DRAMsim3_dir + "HeteroCIM_results/"
            ...
            assert(0), "cannot find dram model backend. Now support HeteroCIM and DRAMsim3 only"
        self.busy = False
